...
import (
	...
	appsummarise "github.com/wingitman/lup/internal/summarise"
	"github.com/wingitman/lup/internal/tui"
...
func tuiCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "tui <file>",
		Short: "Open a two-pane TUI for a document and its lup summary",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			model, err := tui.New(args[0])
			if err != nil {
				return err
			}
...
func summariseCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			generator := llm.NewGenerator(cfg.ActiveAgent(), cfg.Active.Level)
			ctx := gocontext.Background()

			fs, err := summariseFile(ctx, generator, relPath, parsed)
			if err != nil {
				return err
			}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for name, agent := range cfg.Agents {
				fmt.Printf("  %s: provider=%s model=%s base_url=%s api_key=%s env=%s\n", name, agent.Provider, agent.Model, agent.BaseURL, maskAPIKey(agent.APIKey), agent.APIKeyEnv)
			}
			fmt.Printf("\n[embedding]\n")
...
func summariseFile(ctx gocontext.Context, generator appsummarise.Generator, relPath string, parsed *parser.File) (store.FileSummary, error) {
	return appsummarise.File(ctx, generator, relPath, parsed, appsummarise.Options{})
}
